...
// root -b -q 'GammaGenList.cxx( 10, 1)'
//
//This code was adapated from that of others on the project
/////////////////////////////////////////////////////////
...
// ================================================
void g10GammaGenList(int N = 100, int firstEvent = 1,std::string outFileName="./prodlist_gammas_g10_100_a.txt")
{
  // Name of output ASCII file
  std::string outputFileName = outFileName;//"./prodlist_gammas_g10_500_a.txt";

  // Particle PDG (11 = electron, 22 = gamma)
  int     pdg       = 22;
  float   mass      = 0.0;

  // Upper/lower bounds for generated particle energy [GeV]
  float   energyLo  = 0.0015049; // 1.5 MeV
  float   energyHi  = 0.0015051; // ... MeV

  //Create a vector of g10 coordinates:
  using coords = std::vector<vector<float>>;  //defining a vector of vector of floats for coordinates in uBoone
  coords cg10{{-.5,-100.,30.},{-.5,-100.,110.},{-.5,-100.,230.},{-.5,-100.,340.},{-.5,-100.,455.},{-.5,-100.,580.},{-.5,-100.,680.},{-.5,-100.,795.},{-.5,-100.,910.},{-.5,-100.,1000.},{-.5,100.,30.},{-.5,100.,110.},{-.5,100.,230.},{-.5,100.,340.},{-.5,100.,455.},{-.5,100.,580.},{-.5,100.,680.},{-.5,100.,795.},{-.5,100.,910.},{-.5,100.,1000.},{250.5,-100.,30.},{250.5,-100.,110.},{250.5,-100.,230.},{250.5,-100.,340.},{250.5,-100.,455.},{250.5,-100.,580.},{250.5,-100.,680.},{250.5,-100.,795.},{250.5,-100.,910.},{250.5,-100.,1000.},{250.5,100.,30.},{250.5,100.,110.},{250.5,100.,230.},{250.5,100.,340.},{250.5,100.,455.},{250.5,100.,580.},{250.5,100.,680.},{250.5,100.,795.},{250.5,100.,910.},{250.5,100.,1000.},{-.5,-50.,20},{-.5,-50.,1015},{-.5,50.,20},{-.5,50.,1015}};
  ...
  // to choose gamma start-point. Commented: Lariat
  // float lx = 45.5;
  // float ly = 38.;
  // float lz = 88.;
  //float lx = 249.;
  ...
  //Half-Width of g10 points / hotspots in each dimension
  float lx = 1.0; //x is really just a point on the edge of the detector
  float ly = 1.0; //y point range is roughly 20cm for hotspots, we'll try half. Originally 10
  float lz = 1.0; //z points range is roughly 40 cm for hotspots, we'll try half. Originally 20
  ...
  for(int i = 0; i < N; i++){
    ...
    float p = energyLo; //+ rand.Rndm()*(energyHi-energyLo);
  ...
  cout << "Finished generating " << N << " events in file " << outFileName << endl;
  output.close();
}
